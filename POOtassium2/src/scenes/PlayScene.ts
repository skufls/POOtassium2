...
export class PlayScene extends Phaser.Scene{
   ...
   constructor(){
    ...
    playerMthrow!: Phaser.Time.TimerEvent;
    playerthrowSPACE!: Phaser.Time.TimerEvent;
    playerR!: Phaser.Time.TimerEvent;
    ...
    create(){
       ...
       spaceKey.on('down',  () => {
        this.playerMthrow.paused = false });
       spaceKey.on('up',  () => {
        this.playerMthrow.paused = true });
        ...
        this.playerMthrow = this.time.addEvent({
            ...
            startAt: 500,
            callback: ()=>{
                this.poopgroup.poopMthrow(this.player.x, this.player.y, this.player.rotation , this.blockslayer),
                this.player.play("monkey_head");
            },
            loop: true,
            paused: true,
       ...
       var joystickCENTER = this.physics.add.sprite(540,390, "joystick")
       .setVisible(false)
       .setScale(0.8)
       var joystick = this.physics.add.sprite(540,390, "joystick")
       .setScrollFactor(0)
       .setScale(0.8)
       .setAlpha(0.5)
       .setInteractive({ draggable: true })
       .setInteractive()
    .on('dragstart', (pointer: any, dragX: any, dragY: any) =>{
        R++,
        R++
        console.log('R',R)
        console.log('F',F)
    })
    .on('drag', (pointer: any, dragX: number | undefined, dragY: number | undefined) =>{
    ...
    .on('dragend', function(pointer: any, dragX: any, dragY: any, dropped: any){
       ...
       joystick.on('drag',  (pointer: any, dragX: any, dragY: any) => {
        ...
        this.playerR= this.time.addEvent({
            callback: ()=>{
                this.player.rotation = Phaser.Math.Angle.Between(joystickCENTER.x, joystickCENTER.y, joystick.x, joystick.y),
                this.physics.velocityFromAngle(this.player.angle, 150, this.player.body.velocity);
...
class poop extends Phaser.Physics.Arcade.Sprite {
    ...
    Mfling(x: number, y: number, rotation: number, pointer:Phaser.Input.Pointer){
...
class poopgroup extends Phaser.Physics.Arcade.Group{
    ...
    poopMthrow(x: number, y: number, rotation: number, blockslayer: Phaser.Tilemaps.TilemapLayer){
        let poop = this.getFirstDead(false);
        if (poop) {
...
class enemys extends Phaser.Physics.Arcade.Sprite{
    ...
    STOP2: Phaser.Time.TimerEvent;
    STOP: Phaser.Time.TimerEvent;
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, texture: string){
        ...
        this.STOP2= scene.time.addEvent({
            callback: ()=>{
